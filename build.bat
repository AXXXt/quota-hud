...
if exist dist\QuotaHUD-Portable.exe copy /y dist\QuotaHUD-Portable.exe Output\QuotaHUD-Portable-1.0.1.exe >nul
echo [4/4] Inno Setup 制作安装包...
"%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe" QuotaHUD.iss || goto :err
echo.
echo 完成!
echo   安装包: %~dp0Output\QuotaHUD-Setup-1.0.1.exe
echo   便携版: %~dp0Output\QuotaHUD-Portable-1.0.1.exe
goto :eof
:err
echo 构建失败
exit /b 1
